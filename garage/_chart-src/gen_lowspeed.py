...
for val in [0.04,0.08,0.12,0.16]:
    o.append('<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="#2c313d" stroke-width="1"/><text x="%d" y="%.1f" fill="#9aa0aa" font-size="11" text-anchor="end">%.2f</text>'%(PX0,Y(val),PX1,Y(val),PX0-8,Y(val)+4,val))
...
for v in SP:
    o.append('<line x1="%.1f" y1="%d" x2="%.1f" y2="%d" stroke="#2c313d" stroke-width="1"/><text x="%.1f" y="%d" fill="#cdd2dd" font-size="12" text-anchor="middle">%d</text>'%(X(v),PY0,X(v),PY1,X(v),PY1+20,v))
o.append('<text x="%d" y="%d" fill="#9aa0aa" font-size="12" text-anchor="middle">速度 km/h（トップギア巡航・概算）</text>'%((PX0+PX1)//2,PY1+42))
o.append('<text x="30" y="%d" fill="#cdd2dd" font-size="13" font-weight="600" text-anchor="middle" transform="rotate(-90 30 %d)">押し出す力 ＝ トルク/(車重+60kg)</text>'%((PY0+PY1)//2,(PY0+PY1)//2))
# curves
...
for n,pts,r60,w,col,wd,dash,lm in B:
    if lm=="mid":
        v=45; o.append('<text x="%.1f" y="%.1f" fill="%s" font-size="12" font-weight="700">%s</text>'%(X(v),Y(interp(pts,r60*v/60)/(w+R))-10,col,n))
    else:
        v=80; o.append('<text x="%.1f" y="%.1f" fill="%s" font-size="12" font-weight="%s">%s</text>'%(X(v)+6,Y(interp(pts,r60*v/60)/(w+R))+4,col,'700' if '！' in n else '400',n))
# annotation
by=560
...
o.append('</svg>')
open("/Users/uminomae/dev/pjdhiro/.claude/worktrees/develop/garage/assets/lowspeed-pull.svg","w").write("\n".join(o))
