...
def frame(px,py,ti,no):
    o.append('<text x="%d" y="%d" fill="#cdd2dd" font-size="16.2" font-weight="700">%s</text>'%(px,py-26,ti))
    ...
    o.append('<text x="%.1f" y="%d" fill="#9aa0aa" font-size="12" text-anchor="middle">← 30m →</text>'%(px+PADW/2,py+PADH+15))
    o.append('<text x="%d" y="%.1f" fill="#9aa0aa" font-size="12" text-anchor="middle" transform="rotate(-90 %d %.1f)">20m</text>'%(px-10,py+PADH/2,px-10,py+PADH/2))
    o.append('<text x="%d" y="%d" fill="#8ab4e6" font-size="12.6">%s</text>'%(px,py+PADH+33,no))
...
bottle(px,py,15,10)
o.append('<text x="%.1f" y="%.1f" fill="#7fe39a" font-size="12" text-anchor="middle">定常円</text>'%(X(px,15),Y(py,10)+16))
# 2: figure-8
...
o.append('</svg>')
open(os.path.join(OUT,"practice-pad-30x20.svg"),"w").write("\n".join(o))
print("OK")
