...
o.append('<text x="%.1f" y="76" fill="#e6e6e6" font-size="17.2" text-anchor="middle">アルファード=1.0</text>'%one)
...
for v in [0,1,2,3,4,5,6]:
    ...
    o.append('<text x="%.1f" y="%d" fill="#9aa0aa" font-size="16.5" text-anchor="middle">%d×</text>'%(X(v),ay+16,v))
o.append('<text x="555" y="%d" fill="#9aa0aa" font-size="18" text-anchor="middle">アルファードの何倍か（×）</text>'%(ay+34))
for i,(nm,tr,pr,col) in enumerate(rows):
    y=y0+i*step
    o.append('<text x="192" y="%d" fill="%s" font-size="18" text-anchor="end" font-weight="600">%s</text>'%(y+2,col,nm))
    o.append('<rect x="200" y="%d" width="%.1f" height="12" rx="2" fill="#f0883a"/>'%(y-13,X(tr)-200))
    o.append('<text x="%.1f" y="%d" fill="#f0883a" font-size="15">%.2f×</text>'%(X(tr)+5,y-4,tr))
    o.append('<rect x="200" y="%d" width="%.1f" height="12" rx="2" fill="#4a9be8"/>'%(y+2,X(pr)-200))
    o.append('<text x="%.1f" y="%d" fill="#4a9be8" font-size="15">%.2f×</text>'%(X(pr)+5,y+11,pr))
by=ay+48
...
o.append('</svg>')
open(os.path.join(OUT,"weight-ratio.svg"),"w").write("\n".join(o))
