...
**事実として**: シモンドンは、既に出来上がった個体を前提とする伝統的な個体化原理（質料形相論の図式を含む）を拒否し、個体化の先行条件として前個体的な準安定系（metastable system）を置きました。準安定系は、隔たった異質な諸秩序のあいだの差異（disparation）として潜在エネルギーを孕み、個体化はこの差異が提起する問題の「解決（resolution）」として進行します。そして個体はこの解決の結果であると同時に、次の個体化の場（milieu）でもあります（以上は精読済みの Deleuze (1966) による書評 *Pli* 12, pp.43-49 の引用に依拠）。さらに『技術対象の存在様式について』(1958) の結論部では、個体化を経ても前個体的実在の荷（charge de réalité pré-individuelle）は枯渇せず各個体のうちに保持され、発明＝新たな個体化の源泉になるとされます（同書 pp.248-249、精読済み）。なお transduction（局所的な構造解決が隣接領域に伝播する操作。結晶化の種結晶が過飽和溶液の構造化を次々に誘発する過程が原型事例）の本格的な定式化は主論文『形相と情報の概念に照らした個体化』で行われますが、同書は OA が存在せず未精読です。この transduction の要約は標準的な二次文献に基づく背景説明であり、原典からの直接引用ではありません。
