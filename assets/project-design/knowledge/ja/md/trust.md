...
<img src="trust-research-map.svg" alt="信頼調査リサーチマップ" style="width:100%; max-width:700px;">

### 5W1H

**What（何を）**: 「信頼とは何か」を学術文献に基づき多分野横断で調査。心理学・社会学・哲学・経済学・神経科学・HCI/AI・愛着理論・神経現象学など12以上の分野を対象とした。

**Why（なぜ）**: 信頼は「プロジェクトデザイン論」の中核概念のひとつであり、欠損駆動思考との接続を見据えた理論基盤の構築が必要だった。単一分野の定義では捉えきれない多層性を明らかにするため。

**Who（誰が）**: pjdhiro が設計・検証を主導し、Claude（Codex）・ChatGPT・ChatGPT Pro の3エンジンによる広域調査を統合した。

**When（いつ）**: 2026年2月8日（概念基盤 CN-005/006）に着手し、3月26-27日に広域調査と9本の深掘りを集中実施、3月31日に必要十分条件レポートで第一段階を完了した。

**Where（どこに）**: 調査資産は `knowledge/research/trust/`（18ファイル、約340KB）に蓄積。概念基盤は `knowledge/concepts/` の CN-005・CN-006。公開用コンテンツは本稿（`content/knowledge/trust-ja.md`）。

**How（どのように）**: 3つのLLMエンジンに統一プロンプト（PROMPT.md）を投入し、独立した広域レビューを取得。それらを統合レポートにまとめた後、9つのサブテーマ（愛着、AI、神経現象学、修復、自己信頼、ガスライティング、経験分類、AI測定、制度修復）を個別に深掘りした。最終的に必要条件5要素の収束と十分条件の領域依存性を確認した。
