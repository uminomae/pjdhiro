...
export async function startMain() {
  console.log('[yinyang-main] startMain() が呼ばれました');

  try {
    // 1) URL クエリから alg
    const alg = getAlgFromURL();
    console.log(`[yinyang-main] alg = "${alg}" を取得`);
    ...
    console.log('[yinyang-main] Three.js 共通初期化 完了');

    // 3) alg に応じたモジュールを動的読み込みして initialize() を呼ぶ
    await loadAlgModule(alg, context);
    console.log('[yinyang-main] 選択モジュール の初期化完了');

    // 4) アニメーションループ開始
    startLoop(context);
    console.log('[yinyang-main] アニメーションループ を開始');

  } catch (err) {
    console.error('[yinyang-main] startMain() 中にエラー発生:', err);
  }
}